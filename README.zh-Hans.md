...
对于 0.4.1 的桌面使用，建议从 GitHub Release 下载已签名的桌面安装包:macOS 上为
`OpenSquilla-0.4.1-mac-arm64.dmg`，Windows 上为 `OpenSquilla-0.4.1-win-x64.exe`。
Windows 便携版 zip 仍作为面向脚本和便携文件夹工作流的旧版兼容包保留。

| 安装方式 | 适合人群 | 何时使用 |
| --- | --- | --- |
| [桌面安装包](#desktop-installers)**（推荐桌面用户）** | macOS 和 Windows 用户 | 已签名的打包桌面应用 |
...
已签名的桌面应用会内置它所需的运行库，但**终端快速安装**不会安装 Homebrew/系统库。
...
> [!NOTE]
> 预览版构建未签名；以管理员身份启动是受支持的路径。如果出现 SmartScreen，请选择
...
已签名的桌面应用会内置它所需的原生运行库。如果你是通过终端快速安装或从 shell 进行源码安装，
